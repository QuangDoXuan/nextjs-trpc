...
type RestaurantType = RouterOutput['store']['byId'] & { isFavorite: boolean };

const Restaurant = (props: { restaurant: RestaurantType }) => {
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-lg relative cursor-pointer transform transition-transform duration-300 hover:scale-105">
      <Swiper
        pagination={{
          clickable: true,
      ...
      <p className="text-custom-gray text-subTitle">
        {restaurant?.city?.name || ''} · {Number(restaurant?.minPrice) || ''}~
        {Number(restaurant?.maxPrice) || ''}만원
      </p>
